...
describe('CalendarService', () => {
    ...
    beforeEach(() => {
        // Force CapacitorCalendar.createEvent to be a spyable function
        (CapacitorCalendar as any).createEvent = jasmine.createSpy().and.returnValue(Promise.resolve({ id: '1' }));
    ...
    describe('addEvent', () => {
        ...
        it('should return permission error if permission is denied', async () => {
            spyOn(service as any, 'ensureCalendarPermission').and.returnValue(Promise.resolve(false));
    ...
    describe('permissionGranted', () => {
        ...
        it('should return true for object with result containing "grant"', () => {
            expect((service as any).permissionGranted({ result: 'granted' })).toBeTrue();
        });
    ...
    describe('ensureCalendarPermission', () => {
        ...
        it('should return false if checkPermission returns denied on web', async () => {
            ...
            // Spy on Capacitor.getPlatform to simulate web environment
            spyOn(Capacitor, 'getPlatform').and.returnValue('web');

            const result = await (service as any).ensureCalendarPermission();
            expect(result).toBeTrue(); // On web, fallback always returns true
        });


        it('should fallback to true on web if plugin fails', async () => {
            // Make checkPermission throw to simulate plugin failure
            spyOn(CapacitorCalendar, 'checkPermission').and.throwError('Plugin failed');

            // Mock web platform
            spyOn(Capacitor, 'getPlatform').and.returnValue('web');

            const result = await (service as any).ensureCalendarPermission();
            expect(result).toBeTrue(); // On web, fallback returns true
        });
