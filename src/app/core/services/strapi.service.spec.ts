...
describe('StrapiService', () => {
  ...
  it('should fetch suspects', () => {
    const dummyResponse: StrapiResponse<Suspect[]> = {
      data: [
        { id: '1', attributes: { /* Suspect attributes */ } } as any
      ]
    };
    ...
    const req = httpMock.expectOne('https://codeagainstcrime.com/api/suspects?populate=*');
  ...
  it('should fetch a single suspect by documentId', () => {
    const dummyResponse: StrapiResponse<Suspect> = {
      data: { id: 'abc123', attributes: { /* Suspect attributes */ } } as any
    };
    ...
    const req = httpMock.expectOne('https://codeagainstcrime.com/api/suspects/abc123?populate=*');
    expect(req.request.method).toBe('GET');
    req.flush(dummyResponse);
  });
  ...
  it('should generate correct image URL', () => {
    ...
    expect(service.getImageUrl('http://example.com/image.png')).toBe('http://example.com/image.png');
    expect(service.getImageUrl('/uploads/image.png')).toBe('https://codeagainstcrime.com/uploads/image.png');
  });
});
