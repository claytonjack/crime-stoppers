...
import { IonContent, IonDatetime, IonCard, IonCardContent } from '@ionic/angular/standalone';
...
describe('PrivacyModePage', () => {
  ...
  beforeEach(async () => {
    const privacyModeSpy = jasmine.createSpyObj('PrivacyModeService', [], { isEnabled: true });
  ...
  it('should update selectedDate and announce date change', async () => {
    ...
    expect(screenReader.speak).toHaveBeenCalledWith(`You selected ${formattedDate}`);
    expect(screenReader.speak).toHaveBeenCalledWith('No events scheduled for this day');
  });
  ...
  it('should render header and no-events card in template', () => {
    ...
    // Check header text
    expect(compiled.querySelector('app-header')?.textContent).toContain('core.privacyMode.headerTitle');
